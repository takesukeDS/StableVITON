...
class ConditionGenerator(nn.Module):
    def __init__(self, opt, input1_nc, input2_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d,
        ...
        timestep_input_dim = ngf
        time_embed_dim = time_embedding_dim or ngf * 4
        self.time_proj = Timesteps(ngf, True, 0.0)
        ...
        self.PoseEncoder = nn.Sequential(
            ResBlock(input2_nc, ngf, norm_layer=norm_layer, scale='down', temb_channels=time_embed_dim),
            ResBlock(ngf, ngf * 2, norm_layer=norm_layer, scale='down', temb_channels=time_embed_dim),
            ResBlock(ngf * 2, ngf * 4, norm_layer=norm_layer, scale='down', temb_channels=time_embed_dim),
            ResBlock(ngf * 4, ngf * 4, norm_layer=norm_layer, scale='down', temb_channels=time_embed_dim),
            ResBlock(ngf * 4, ngf * 4, norm_layer=norm_layer, scale='down', temb_channels=time_embed_dim)
        )

        self.conv = ResBlock(ngf * 4, ngf * 8, norm_layer=norm_layer, scale='same', temb_channels=time_embed_dim)

        if opt.warp_feature == 'T1':
            # in_nc -> skip connection + T1, T2 channel
            self.SegDecoder = nn.Sequential(
                ResBlock(ngf * 8, ngf * 4, norm_layer=norm_layer, scale='up', temb_channels=time_embed_dim),  # 16
                ResBlock(ngf * 4 * 2 + ngf * 4, ngf * 4, norm_layer=norm_layer, scale='up',
                         temb_channels=time_embed_dim),  # 32
                ResBlock(ngf * 4 * 2 + ngf * 4, ngf * 2, norm_layer=norm_layer, scale='up',
                         temb_channels=time_embed_dim),  # 64
                ResBlock(ngf * 2 * 2 + ngf * 4, ngf, norm_layer=norm_layer, scale='up', temb_channels=time_embed_dim),
                # 128
                ResBlock(ngf * 1 * 2 + ngf * 4, ngf, norm_layer=norm_layer, scale='up', temb_channels=time_embed_dim)
                # 256
            )
        if opt.warp_feature == 'encoder':
            ...
            self.SegDecoder = nn.Sequential(
                ResBlock(ngf * 8, ngf * 4, norm_layer=norm_layer, scale='up', temb_channels=time_embed_dim),  # 16
                ResBlock(ngf * 4 * 3, ngf * 4, norm_layer=norm_layer, scale='up', temb_channels=time_embed_dim),  # 32
                ResBlock(ngf * 4 * 3, ngf * 2, norm_layer=norm_layer, scale='up', temb_channels=time_embed_dim),  # 64
                ResBlock(ngf * 2 * 3, ngf, norm_layer=norm_layer, scale='up', temb_channels=time_embed_dim),  # 128
                ResBlock(ngf * 1 * 3, ngf, norm_layer=norm_layer, scale='up', temb_channels=time_embed_dim)  # 256
            )
        if opt.out_layer == 'relu':
            self.out_layer = ResBlock(ngf + input1_nc + input2_nc, output_nc, norm_layer=nn.InstanceNorm2d,
                                      scale='same',
    ...
    def zero_conv_out(self):
        zero_module(self.flow_conv)

    def forward(self, input1, input2, timesteps, upsample='bilinear'):
        if len(timesteps.shape) == 0:
            timesteps = timesteps[None].to(input1.device)
        timesteps = timesteps.expand(input1.shape[0])
        t_emb = self.time_proj(timesteps)
        t_emb = t_emb.to(dtype=input1.dtype)
        ...
        for i in range(5):
            if i == 0:
                ...
                E2_list.append(self.PoseEncoder[i](input2, emb))
            else:
                E1_list.append(self.ClothEncoder[i](E1_list[i - 1]))
                E2_list.append(self.PoseEncoder[i](E2_list[i - 1], emb))

        # Compute Clothflow
        for i in range(5):
            N, _, iH, iW = E1_list[4 - i].size()
            grid = make_grid(N, iH, iW, input1.device)
            ...
            if i == 0:
                ...
                if self.warp_feature == 'T1':
                    x = self.SegDecoder[i](torch.cat([x, E2_list[4 - i], warped_T1], 1), emb)
                if self.warp_feature == 'encoder':
                    warped_E1 = F.grid_sample(E1_list[4 - i], flow_norm + grid, padding_mode='border')
                    x = self.SegDecoder[i](torch.cat([x, E2_list[4 - i], warped_E1], 1), emb)
        ...
        x = self.out_layer(torch.cat([x, input2, warped_input1], 1), emb)
...
def make_grid(N, iH, iW, device):
    ...
    grid = torch.cat([grid_x, grid_y], 3).to(device)
    return grid
...
class ResBlock(nn.Module):
    def __init__(self, in_nc, out_nc, scale='down', norm_layer=nn.BatchNorm2d,
        ...
        if temb_channels is not None:
            self.time_emb_proj = nn.Linear(temb_channels, out_nc)
        else:
            self.time_emb_proj = None

    def forward(self, x, temb=None):
        residual = self.scale(x)
        hidden_states = self.block1(residual)
        if self.time_emb_proj is not None:
            hidden_states += self.time_emb_proj(temb)[:, :, None, None]
        hidden_states = self.block2(hidden_states)
        return self.relu(residual + hidden_states)
